...
fn test_parse_sample_data() {
    let sample_data =
        fs::read_to_string("examples/sample.json").expect("Failed to read sample.json");
    ...
    for line in sample_data.lines().take(10) {
        let flow: FlowMessage = serde_json::from_str(line).expect("Failed to parse flow message");
...
fn test_ipv6_traffic_parsing() {
    ...
    assert_eq!(
        flow.dst_addr,
        Some("2602:47:d4ad:f901:aa23:feff:fe20:946b".to_string())
    );
...
async fn test_application_components() {
    ...
    stdin_reader::process_reader(reader, metrics.clone())
        .await
        .unwrap();
